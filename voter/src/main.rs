...
use blind_rsa_signatures::{
    BlindSignature, BlindedMessage, MessageRandomizer, Options, PublicKey as RSAPublicKey, Secret,
    Signature,
};
...
// Demo RSA public key for the electoral commission:
...
struct App {
    ...
    blinded_h_n: Option<BlindedMessage>, // Blinded hash of the nonce
...
fn load_ec_pubkey() -> Result<RSAPublicKey> {
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let pk = load_ec_pubkey().expect("Failed to load EC public key");
                ...
                if let Kind::GiftWrap = event.kind {
                    ...
                    match message.kind {
                        1 => {
                            ...
                            let secret = app.secret.as_ref().unwrap();
                            let msg_rand = *app.r.as_ref().unwrap();
